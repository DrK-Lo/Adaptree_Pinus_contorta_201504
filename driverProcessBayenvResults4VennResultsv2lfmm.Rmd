...
R
rmarkdown::render("driverProcessBayenvResults4VennResultsv2lfmm.Rmd")
...
In this analysis, we ran lfmm (Frichot et al 2013) on over 1,000,000 SNPs from the Pine sequence capture.  We had 22 environmental variables of biological interest.
...
```{r read in file}
filename <- "bayenv.lfmmbeagle.res"
...
### append missing data column to file
num_ind <- read.table("num_ind.txt", header=TRUE)
head(num_ind)
nrow(num_ind)
unit1 <- identical(paste(num_ind$gcontig, num_ind$pos_gcontig, sep="__"), paste (f2$gcontig, f2$pos_gcontig, sep="__"))
if (unit1==FALSE){print("Error: data frame with num ind sequenced does not line up")}
```
...
### read in file with cutoffs
  cos <- read.table("zscore_cutoffs", header=TRUE)
